...
def main():
    print("--- Online Retail Customer Clustering ---")
    ...
    try:
        df = pd.read_csv(files[0], encoding='unicode_escape')
        if df.shape[1] == 1:
    ...
    # Handle specific aggregations
    
    if 'CustomerID' in df.columns:
        print("Aggregating Online Retail data by CustomerID...")
        df = df.groupby('CustomerID').agg({'Quantity': 'sum', 'UnitPrice': 'mean'}).reset_index()

    
    # Drop irrelevant identifier columns
    cols_to_drop = [c for c in df.columns if str(c).lower() in ['id', 'customerid', 'user_id', 'passengerid', 'name', 'invoiceno', 'stockcode', 'description', 'country', 'invoice', 'date']]
